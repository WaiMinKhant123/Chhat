...
@WebServlet("/AAdmin_Dashboard")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 2MB
                 maxFileSize = 1024 * 1024 * 10,      // 10MB
                 maxRequestSize = 1024 * 1024 * 50)   // 50MB
public class AAdmin_Dashboard extends HttpServlet {
    ...
    private void editNews(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            ps.executeUpdate();
            response.sendRedirect("mananageAD.jsp");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (inputStream != null) {
    ...
    private void viewNews(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                RequestDispatcher dispatcher = request.getRequestDispatcher("View_newsAD.jsp");
    ...
    private void showEditForm(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                request.setAttribute("date", rs.getString("date"));
                request.setAttribute("content", rs.getString("content"));
                request.getRequestDispatcher("ADedit.jsp").forward(request, response);
            }
